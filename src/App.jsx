...
function TreasuryScreen({ user }) {
  ...
  const load = async () => {
    setLoading(true);
    const [treasRes, txRes] = await Promise.all([
  ...
  const handleSave = async () => {
    if (!form.amount) { setErrorMsg("المبلغ مطلوب"); return; }
    setSaving(true); setErrorMsg("");

    const treas = treasury.find(t => t.branch_id === (user?.branchId || 1));
    if (!treas) { setErrorMsg("لم يتم العثور على خزنة الفرع"); setSaving(false); return; }

    const newBalance = form.type === "قبض"
      ? Number(treas.balance) + Number(form.amount)
      : Number(treas.balance) - Number(form.amount);

    if (newBalance < 0 && form.type === "صرف") {
      setErrorMsg("رصيد الخزنة غير كافي"); setSaving(false); return;
    }
    ...
    if (!txError) {
      await supabase.from("treasury").update({ balance: newBalance }).eq("id", treas.id);
    }
  ...
  const refTypeColor = (ref) => {
    if (ref === "مبيعات") return "blue";
    if (ref === "مشتريات") return "red";
    if (ref === "بنك") return "gold";
    return "gray";
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        <Card className="p-4 border-r-4 border-green-500 bg-green-50">
          <p className="text-2xl font-black text-green-700">{totalIn.toLocaleString()} ج</p>
          <p className="text-sm text-green-600">إجمالي المقبوضات</p>
        </Card>
        <Card className="p-4 border-r-4 border-red-500 bg-red-50">
          <p className="text-2xl font-black text-red-700">{totalOut.toLocaleString()} ج</p>
      ...
      {showForm && (
        <Card className="p-5 border-2 border-blue-200 bg-blue-50/30">
          ...
          <div className="grid grid-cols-2 gap-4">
              ...
              <div className="flex gap-3">
                {[["قبض", "green", "↓"], ["صرف", "red", "↑"]].map(([type, color, arrow]) => (
                  <button key={type} onClick={() => setForm({ ...form, type })}
                    ...
                    <span className="text-lg">{arrow}</span> {type}
                  </button>
                ))}
              </div>
            </div>
            ...
            <div>
              <label className="block text-sm font-semibold text-slate-700 mb-1">المصدر / الجهة</label>
      ...
      <Card className="p-4">
          ...
          <div className="flex gap-2">
            {[["all", "الكل"], ["قبض", "مقبوضات"], ["صرف", "مصروفات"]].map(([val, label]) => (
              <button key={val} onClick={() => setFilter(val)}
                className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${filter === val ? "bg-blue-600 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}>
                {label}
              </button>
            ))}
          </div>
        ...
        {loading ? <div className="text-center py-8 text-slate-400 text-sm">جاري التحميل...</div>
          : filtered.length === 0 ? <div className="text-center py-8 text-slate-400 text-sm">لا توجد حركات بعد</div>
          : (
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                ...
                <tbody>
                  {filtered.map(t => (
                    <tr key={t.id} className="border-b border-gray-50 hover:bg-gray-50">
                      ...
                      <td className="py-3 px-3">
                        <Badge color={t.type === "قبض" ? "green" : "red"}>{t.type}</Badge>
                      </td>
                      <td className="py-3 px-3 text-slate-700">{t.description || "—"}</td>
                      <td className="py-3 px-3">
                        <Badge color={refTypeColor(t.reference_type)}>{t.reference_type || "يدوي"}</Badge>
                      </td>
...
function SalesScreen({ user }) {  const [sales, setSales] = useState([]);
  ...
  const emptyForm = { invoice_number: "", customer_id: "", notes: "", discount: "0", paid: "0", payment_type: "آجل" };
  const [form, setForm] = useState(emptyForm);
...
function PurchasesScreen() {
  ...
  const emptyForm = { invoice_number:"", supplier_id:"", notes:"", discount:"0", paid:"0", payment_type:"آجل" };
  const [form, setForm] = useState(emptyForm);
  const [items, setItems] = useState([{ product_id:"", quantity:"", unit_price:"" }]);
  ...
  const handleSave = async () => {
    ...
    const payload = { invoice_number:form.invoice_number, supplier_id:form.supplier_id, branch_id: user?.branchId || null, payment_type:form.payment_type, total:netTotal, paid:form.payment_type==="نقدي"?netTotal:Number(form.paid||0), discount:Number(form.discount||0), notes:form.notes, status:form.payment_type==="نقدي"?"مكتمل":status };
